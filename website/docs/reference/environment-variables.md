...
| `HERMES_KANBAN_RUN_ID` | Set by the kanban dispatcher alongside `HERMES_KANBAN_TASK` — the `task_runs` row the spawned worker owns. It is the worker's run-ownership pin: a lifecycle write (failure / timeout / termination) whose id is no longer the task's `current_run_id` is refused, so a worker that outlives its own run can't close a successor's. Don't set manually. |
