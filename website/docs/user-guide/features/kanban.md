...
| `default_assignee` | `""` | Where a child task lands when the LLM picks an unknown profile. Empty = fall back to active default. Also applies to cards *you* create without `--assignee` on the CLI: when set, the dispatcher auto-assigns any unassigned `ready` card to this profile on its next tick. `hermes kanban create` warns at creation time when this will happen (and notes when an unassigned card will sit idle instead). Use `--hold` to create a card the dispatcher will never touch until you `hermes kanban unblock` it. The dispatcher also never fires more workers at a profile than that profile's own `max_concurrent_sessions` allows — the effective ceiling is the lower of it and `kanban.max_in_progress_per_profile`, cards over it are deferred (not failed) to a later tick, and the dispatcher logs them as `session_capped`. |
